...
WYSIŁKIEM jest słownik”. Być może potrzebna Ci będzie dodatkowa zmienna pomocnicza.
...
1. Utworzyłam pliki `index.html` i `script.js` dla rozdzielenia kodu (dla wyświetlania efektu w `body`).
    1. **HTML**
        1. W `head` dodałam:
            1. stylowanie dla tekstu (`font-weight`, `font-size` i `font-family`),
            2. dodałam `script src=./script.js` z opcją `defer`,
    ...
    2. **JS**
        1. Przygotowałam zmienne:
            ...
            4. zmienną `eff` dla uporządkowania tablicy jako zdanie.
2. Kolejność działań
    1. Z pomocą zmiennej `help` uporządkowałam zmienne `zm1..zm9` do kolejności w treści zadania.
