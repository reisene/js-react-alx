...
Prawidłowo zweryfikowane zmienne przekazuję do funkcji `oblicz(a, b, c)`. Rodzaj wykonywanej operacji matematycznej obsługuję za pomocą instrukcji `switch(c)`. W przypadkach dzielenia (`/`) oraz reszty z dzielenia (`%`) dodałam warunek `if (a ===0 || b === 0)`, który w razie próby dzielenia przez zero przerywa działanie i wyświetla komunikat o błędzie.
